...
def main(config, args):
    ...
    early_stopping_ema = EarlyStopping(
        patience=args.early_stopping_patience,
        min_delta=args.early_stopping_min_delta,
        verbose=True
    ) if args.model_ema else None
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        # Check early stopping for main model
        if early_stopping(acc1):
            logger.info(f"Early stopping triggered after {epoch + 1} epochs!")
            logger.info(f"No improvement in validation accuracy for {args.early_stopping_patience} consecutive epochs.")
            break
        ...
        if model_ema is not None:
            ...
            # Check early stopping for EMA model
            if early_stopping_ema(acc1_ema):
                logger.info(f"Early stopping triggered for EMA model after {epoch + 1} epochs!")
                logger.info(f"No improvement in EMA validation accuracy for {args.early_stopping_patience} consecutive epochs.")
                # Note: We continue training the main model even if EMA early stops
